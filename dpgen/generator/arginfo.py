...
def model_devi_lmp_jobs_args() -> List[Argument]:
...
def model_devi_calypso_jobs_args() -> List[Argument]:
    doc_times = ('List of int. Iteration index. Different number in times List means different iteration index'
                 'Iterations mentioned in List wil use same CALYPSO parameters defined in model_devi_jobs.')
    doc_name_of_atoms = 'List of String. Parameter of CALYPSO input file, means the element species of structures to be generated.'
    doc_number_of_atoms = 'List of int. Parameter of CALYPSO input file, means the number of atoms for each chemical species in one formula unit.'
    doc_number_of_formula = 'List of int. Parameter of CALYPSO input file, means the range of formula unit per cell.'
    doc_volume = 'Parameter of CALYPSO input file, means the colume per formula unit(angstrom^3).'
    doc_distance_of_ion = 'List of List[float]. Parameter of CALYPSO input file, means minimal distance between atoms of each chemical species. Unit is in angstrom.'
    doc_pso_ratio = 'Parameter of CALYPSO input file, means the proportion of the structures generated by PSO.'
    doc_pop_size = 'Parameter of CALYPSO input file, means the number of structures to be generated in one step in CALYPSO.'
    doc_max_step = 'Parameter of CALYPSO input file, means the number of max step in CALYPSO.'
    doc_icode = 'Parameter of CALYPSO input file, means the chosen of local optimization, 1 is vasp.'
    doc_split = 'Parameter of CALYPSO input file, means that generating structures and optimizing structures are split into two parts, in dpgen workflow, Split must be T.'
    doc_vsc = ('Parameter of CALYPSO input file. If this tag is set as “True”, structure prediction of automatic variation of'
               'chemical compositions will be performed. This technique is designed to explore all possible'
               'stoichiometries for given binary systems (e.g., AxBy system) at once.')
    doc_max_num_atoms = ('Parameter of CALYPSO input file. The maximal number of atoms allowed in the simulation cell.'
                        'only works with vsc.')
    doc_ctrl_range = ('Parameter of CALYPSO input file. Defining the compositional range for each type of constituent atom in the binary AxBy system.'
                      'only works with vsc.')
    doc_pstress = 'Same as PSTRESS in INCAR.'
    doc_fmax = ('The convergence criterion of structure optimization with dp and ase.'
                'It means that the force on all individual atoms should be less than fmax.')

    args = [
        Argument("times", list, optional=False, doc=doc_times),
        Argument("NameOfAtoms", list, optional=False, doc=doc_name_of_atoms),
        Argument("NumberOfAtoms", list, optional=False, doc=doc_number_of_atoms),
        Argument("NumberOfFormula", list, optional=True, doc=doc_number_of_formula),
        Argument("Volume", float, optional=True, doc=doc_volume),
        Argument("DistanceOfIon", list, optional=False, doc=doc_distance_of_ion),
        Argument("PsoRation", float, optional=True, doc=doc_pso_ratio),
        Argument("PopSize", int, optional=True, doc=doc_pop_size),
        Argument("MaxStep", int, optional=True, doc=doc_max_step),
        Argument("Icode", int, optional=True, doc=doc_icode),
        Argument("VSC", str, optional=True, doc=doc_vsc),
        Argument("MaxNumAtoms", int, optional=True, doc=doc_max_num_atoms),
        Argument("CtrlRange", list, optional=False, doc=doc_ctrl_range),
        Argument("PSTRESS", float, optional=True, doc=doc_pstress),
        Argument("fmax", float, optional=True, doc=doc_fmax),
    ]

    doc_model_devi_jobs = 'Settings for exploration in 01.model_devi. Each dict in the list corresponds to iterations in times List. '
    return Argument("model_devi_jobs", list, args, [], repeat=True, doc=doc_model_devi_jobs)


def model_devi_calypso_args() -> List[Argument]:
    """CALYPSO engine arguments."""
    doc_calypso_input_path = ("The absolute path of CALYPSO input file named input.dat(PSTRESS and fmax should be included)."
                              "When this key exists, all the iters will use the same CALYPSO input file "
                              "until reach the number of max number of iteration specified by model_devi_max_iter "
                              "and model_devi_jobs key will not work.")
    doc_model_devi_max_iter = "Max number of DPGEN iteration, it works when calypso_input_path exists."
    doc_vsc = ("If the species of structures you intend to research is bigger than two(such as Mg-Al), this mode could enable element-, binary- or ternary- structure searching with CALYPSO in one iteration."
               "which is an efficent way to improve the variety of dataset.")
    ...
    return [
        ...
        Argument("vsc", bool, optional=True, doc=doc_vsc),
    ]
...
def model_devi_args() -> List[Variant]:
    ...
    doc_calypso = "CALYPSO engine. The command argument in the machine file should be path to sander."
